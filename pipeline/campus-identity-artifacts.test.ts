...
test('PostgreSQL staged publishing is repeatable, refreshes links and refuses active mutation', { skip: !connection }, async () => {
  ...
  try {
    ...
    assert.equal(Number((await client.query('SELECT count(*) AS n FROM rockygpt_v2.release_artifacts')).rows[0].n), 6);
    ...
    assert.equal(repeated.rows.length, 7); // Six identity artifacts plus original clubs.
