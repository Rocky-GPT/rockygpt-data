...
export type IdentityCollection = 'contacts' | 'campus_hours' | 'dining_hours' | 'menu' | 'faculty' | 'programs' | 'courses';
...
const COLLECTIONS = ['contacts', 'campus_hours', 'dining_hours', 'menu', 'faculty', 'programs', 'courses'];
...
function reference(value: unknown, evidence = false): void {
  const ref = object(value, ['collection', 'source_key', 'source_record_key', ...(evidence ? ['field'] : [])], 'Record reference', evidence ? ['source_url'] : []);
...
export function validateCampusIdentities(value: unknown): asserts value is CampusIdentities {
  ...
  const ids = new Set<string>(); const records = new Set<string>();
  for (const entry of registry.entities) {
    ...
    ids.add(entity.id);
    if (!['office', 'person', 'facility', 'venue', 'program'].includes(entity.kind as string)) throw new Error('Unsupported identity kind.');
    ...
    for (const entry of entity.links) {
      const link = object(entry, ['collection', 'source_key', 'source_record_keys'], 'Identity link', ['selector']);
  ...
  for (const entity of registry.entities as CampusIdentity[]) for (const relation of entity.relationships || []) {
    if (relation.type === 'convener' && !ids.has(relation.target_entity_id)) throw new Error('Broken relationship identity target.');
  }
}
