...
const [snapshotPath, outputPath, rawProgramsPath, rawClubsPath, rawEventDetailsPath, campusMapPath = 'data/map/campus-map-data.json'] = process.argv.slice(2);
if (!snapshotPath || !outputPath) throw new Error('Usage: tsx pipeline/commands/compile-identities.ts SNAPSHOT_JSON OUTPUT_DIR [CATALOG_RAW_JSON] [CLUBS_RAW_JSON] [EVENT_DETAILS_RAW_JSON] [CAMPUS_MAP_JSON]');
...
const result = compileCampusIdentities(seed, read(snapshotPath) as IdentitySnapshot, rawPrograms, { clubs: rawClubsPath ? read(rawClubsPath) : undefined, eventDetails: rawEventDetailsPath ? read(rawEventDetailsPath) : undefined, campusMap: read(campusMapPath) });
