...
On September 24, 2026, an offline compile of the dev release with `LC` and the Library's reviewed location has 218 `office_at` and 10 `located_at` relationships: the 17 Library staff with `LC-` rooms and the Library itself are placed in the Peter P. Mercer Learning Commons. The three rooms above stay unresolved.
