...
const [snapshotPath, outputPath, rawProgramsPath] = process.argv.slice(2);
if (!snapshotPath || !outputPath) throw new Error('Usage: tsx pipeline/commands/compile-identities.ts SNAPSHOT_JSON OUTPUT_DIR [CATALOG_RAW_JSON]');
...
const result = compileCampusIdentities(seed, read(snapshotPath) as IdentitySnapshot, rawPrograms);
