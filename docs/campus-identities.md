...
On September 24, 2026, 23 more aliases resolve 91 links on the dev release: 13 conveners and 78 Program Faculty listings. Each old URL was followed hop by hop, and each target profile loaded and named its person.
...
- 2 name the person differently: a middle initial, and an old `/hgs/` URL that WordPress completes to her full name. A person approved them.
...
15 listings stay unresolved; their people have no person identity, and at least one has left Ramapo. Each alias's `evidence` says which basis applies.
...
Each school's aliases are its abbreviation and its legacy names. The Archway directory groups that publish a former school name ("School of Contemporary Arts", "School of Humanities and Global Studies") the Anisfield group and, since September 24, 2026, the groups named after the School of Science, Nursing, and Health and the School of Social Sciences and Social Work are linked to their current school by exact record key, as reviewed in the file. They are no longer separate organization identities.
...
Directory categories `Student Organization`, `Honor Society`, `Greek Life`, and `Office Sponsored Organization` become `club` identities. Every other directory group (departments, teams, residence halls, schools and seminars) becomes an `organization` identity, under the same group-ID bridge. Both kinds derive their ID from the Archway group ID with the same `club` label, so a group Archway recategorizes keeps its ID; the continuity check treats a club becoming an organization, or back, as the same group. A non-club group whose name exactly matches a reviewed identity's name or alias, or the department that a reviewed office, facility or venue's own contact record publishes, gets no second identity: it is reported as needing a reviewed link, so that name keeps resolving to the reviewed record. Directory groups are never merged with offices or facilities by name. A reviewed link names both sides instead: `archway_groups` in `src/reference/campus-identity-reviews.json` gives the office by persistent ID and the group by exact record key, and the group becomes the office's own `clubs` link. Since September 24, 2026 that covers *Potter Library* (the Library), *Counseling Services* (Counseling Center), *Center for Student Involvement*, *Center for Student Success* (its Academic Advising office) and *Public Safety* (the non-emergency office; the emergency entry stays the emergency line). Each group's name is the department its office's own directory entry publishes. That department is a `department` alias of the office, so where the group's name differs from the office's, the Brain's fact reader counts both as one name: the office's `name` fact gives the office's name, and the group's published name stays in that fact's evidence with a caveat. A name no alias a person wrote or the directory published covers still conflicts.
